...
2. 在“简历与资料”选择 PDF / TXT / MD / JSON，审阅解析结果，逐项确认并保存。文字版 PDF 直接提取，文字较少的页面自动使用 macOS Vision 本地 OCR；版本信息会显示识别来源及 OCR 页数。资料值放在原有 Keychain，附件保存在原有数据目录。
3. 在“投递工作台”导入 CSV / TSV / XLSX，选择一个岗位，点击“准备填写”，选择本次简历版本，在任务工作区核对并确认本次披露范围后继续。导入不触发申请。
4. 需要登录时在专用 Chrome 正常登录，再点击“已完成登录，重新检查”。需要资料时直接在任务工作区回答，选择“仅本次申请”或“通用资料”。经历先绑定到具体资料记录；未识别/敏感控件仍在官网人工处理。
5. 本人在官网核对并最终提交后，点击“我已在官网提交，检查回执”。只有既匹配回执又匹配岗位编号才记录系统确认；未验证保留 `UNKNOWN_RESULT`，不允许盲目再次填写。也可单独记录本人在官网核查的结果，证据标记为人工确认。
6. 打开“设置与连接”，客户端空闲时自动查找并验证飞书 CLI，保存绝对路径。点击“检测连接”检查 Chrome、飞书授权和目标表；填写并保存实际 Base token / Table ID，完成官方授权后验证，再到工作台点击“同步飞书”。官网与飞书登录互相独立，未配置模型不影响规则填写。

岗位缺少官网入口时，可在详情中补充。配置站点规则使用“导入站点规则”，原有规则文件格式没有变化。未适配站点可用保守的通用填写，但可靠登录验证、进度查询和回执识别仍需对应规则；界面会如实显示待配置。

飞书模板以公司、投递岗位、投递渠道、投递日期、投递状态、岗位链接和备注为主要列。在“设置与连接”展开“投递跟踪模板与视图”，可为已保存的新版目标表配置 Grid、投递状态看板和投递清单；先在任务详情确认影响，后台完成配置后从飞书回读验证。不会因打开设置页自动配置视图，新建主表仍沿用 CLI。字段、状态及旧表兼容说明见[投递跟踪模板](feishu-template.md)。
...
设置页空闲时会自动从 PATH、Homebrew、用户本地目录和 nvm 等常见安装位置查找 `lark-cli`，包括自定义 `NVM_DIR`。只有实际运行 `--version` 并确认 CLI 身份后才保存绝对路径。自动查找在工作进程中执行，共用跨进程锁；检测单个候选最多 3 秒，全部版本探测最多 9 秒。切页与刷新共用本次进程的结果，重启客户端后重新验证，避免重复启动子进程。

“可执行文件已验证”表示本机 CLI 能运行。随后设置页会通过 `auth status --json --verify` 检查已有登录，授权状态与表结构分别展示；不会自动发起新的授权。授权结果摘要单独保存在本地，保存其他配置不再清空该状态；重启后再次打开设置会重新验证，网络或 CLI 检测失败显示“暂时无法验证”，不当作授权已失效。实际登录凭证由官方 CLI 保存，退出客户端不会清除。

“验证已有授权”可随时重新检查登录，无需先开始新的授权。确实需要登录时点击“开始官方授权”，在飞书页面确认，再点“完成本次授权”；完成后显示明确的成功提示。尚未完成的临时授权请求只在本次进程有效。“检测连接”另外检查目标表权限和结构。CLI 未安装、所选路径失效、版本不匹配、找不到 Node 或超时会显示具体原因；保留已保存的路径，点击“重新查找”才会搜索替代安装，也可以使用原生文件选择器指定。

Finder 启动时使用包内独立 Node 执行 npm 安装的 CLI，不依赖登录 Shell 的 PATH，也不读取 Shell 初始化文件。Chrome 可执行文件路径同样可选择并保存。
...
- UI 首次读取快照，再注册唯一事件订阅并补读一次，覆盖订阅瞬间的变化。事件通知只触发快照读取；刷新、切页、重新进入任务工作区不会启动任务。组件卸载会撤销监听。
...
“简历与资料”支持独立 PDF 版本、命名与默认版本切换；工作台启动填写时可为本次岗位另选版本，任务工作区展示绑定的版本与修订号。详见[多版本简历与岗位选择](resume-versions.md)。


## 行动优先的界面

工作台按“公司与岗位 → 招聘进展 → 下一步 → 最近记录与来源 → 操作”阅读。复杂任务在工作台的二级页面处理，返回列表会保留搜索、筛选和滚动位置。资料页支持分组编辑与识别来源对照；采用候选只更新草稿，确认并保存才可用于填写。设置按连接概览、站点规则、飞书同步、模型辅助、每日查询、数据与隐私分类。

详细接线、分组规则、验证范围与截图见 [UI 改造与验收说明](ui-redesign.md)。
