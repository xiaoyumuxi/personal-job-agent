...
“可执行文件已验证”只表示本机 CLI 能运行，飞书授权与表格验证状态单独展示。自动查找不发起授权、不写入飞书、不安装调度。“检测连接”才会调用已有 doctor 验证授权与表结构。CLI 未安装、所选路径失效、版本不匹配、找不到 Node 或超时会显示具体原因；保留已保存的路径，点击“重新查找”才会搜索替代安装，也可以使用原生文件选择器指定。
