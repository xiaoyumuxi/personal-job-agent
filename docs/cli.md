...
CLI 与 macOS 客户端共用业务模块、SQLite、专用 Chrome 目录和 Keychain。日常界面操作见 [客户端指南](desktop.md)，项目安装入口见 [README](../README.md)。本页提供命令行配置与高级操作。
...
npm run dev -- jobs channel JOB_ID 'https://careers.example.com/apply?ref=YOUR_REFERRAL'
```

将示例 URL 替换为本人核对过的真实官网入口，保留原有内推参数。
...
腾讯招聘表可直接导入，无需修改原表头：企业名称 → 公司，招聘岗位 → 岗位，内推类型 → 招聘批次，内推链接/官网 → 投递入口，内推码（含大小写说明） → 内推信息，投递注意事项 → 备注。表头中的全半角括号说明不影响匹配；同时存在展示链接与「完整地址」列时优先使用完整地址，显式映射优先级最高。行业、地点、毕业要求、公司介绍和附件地址等其他列保留在岗位详情的「招聘信息」中。

导入器兼容带 XML 命名空间前缀、绝对内部关系路径的 XLSX。个别记录的链接无效时仍导入该记录，保留原始值并标记为待补充入口，导入结果显示待补充数量。原表文件不会被改写。
...
详细字段、实际 CLI 版本核查、默认模板来源、整行填色步骤及未验证范围见 [飞书说明](feishu.md)。本版只写一张主表，不发送飞书消息。
...
安装前展示本机时区、执行时间、Chrome 窗口与漏跑补查影响，并要求本人确认。使用用户 LaunchAgent；只有真实 `launchctl bootstrap` 成功才报告 installed。初始化项目或打开客户端不会自动安装后台任务。
...
实际命令、数量与结果详见 [验证记录](verification.md)。
