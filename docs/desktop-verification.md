...
## 多版本简历补充验收（2026-09-08）

新增独立的简历版本、命名、默认版本切换与每次岗位填写前的版本选择。资料继续使用原 schema、Keychain 与附件目录；申请记录保存选定版本和修订号，恢复时校验版本是否已改变。

- 从独立源码目录构建，排除共享工作区中同时进行的其他改动；显式使用 Node 24.14.0。
- `npm run typecheck`、`npm run desktop:package`：通过。本机 arm64 应用已构建。
- `npm test`：**78/78 通过**。新增 7 项版本回归，覆盖不同简历的数据/经历/确认状态隔离、旧资料备份、内容去重、显式更新冲突、Keychain 写入失败、跨进程锁、任务版本固定、修订变更后的恢复保护，以及并发读写时重新读取已提交版本。
- `npm run test:browser`：**18/18 通过，15.0 秒**。原字段匹配、登录、附件上传、人工修改保护和暂停恢复通过。
- `JOBAGENT_TEST_PACKAGE=1 npm run test:desktop`：**9/9 通过，最终一轮 49.9 秒**，同时覆盖开发版与打包应用；新增两个多 PDF 场景使用真实 Keychain、受控文件导入、PDF 解析、专用 Chrome 和原填写引擎。
- 新场景分别导入两份虚构 PDF，确认姓名、重命名、切换、刷新并关闭重开；默认选 A、岗位选 B 时，实际网页收到 B 的姓名，上传文件字节与 B 的 PDF 完全一致。
- 打开版本选择框不会启动任务；实际启动后拒绝切换版本。任务中将补充邮箱保存到所选 B 版本，A 的邮箱保持原值；暂停、恢复、刷新保留同一 runId，重启后申请的版本 ID 和默认版本分别保留。
- 独立安装包交付于 `release/resume-versions/JobAgent-darwin-arm64/JobAgent.app`，包内关键代码、渲染资源与 Node 已与测试构建逐字节核对。
- 已人工查看测试环境中的打包客户端截图，版本控件、文件原名和确认状态显示正常；`localStorage.length === 0`。所有 PDF、网页和个人值均为显式测试数据，临时 Keychain 条目在测试结束清理。

本次没有导入用户真实简历、写入真实申请或访问真实招聘表单。旧版导入历史没有资料快照，无法自动恢复各次旧 PDF 的独立经历；使用者需要重新导入需要分开管理的 PDF。真实官网端到端投递、Finder 人工打开、跨机器分发与 Developer ID 签名公证仍未验证。
