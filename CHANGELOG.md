...
- 工作台增加“官网找岗位”二级页面，接入腾讯社招和字节校招／实习的关键词搜索与完整 JD 读取。
- 选择单个简历版本，预览已确认资料并单独确认模型披露；本地条件和带原文引用的 AI 判断分开，失败与未知结果明确保留。
- 支持暂停、取消保留结果、结果过滤、原文对照和显式加入工作台；推荐不创建申请，不触发同步或提交。
- 新增采集／匹配／IPC／renderer 回归，DMG 内 App 也验证推荐入口与启动预览。
