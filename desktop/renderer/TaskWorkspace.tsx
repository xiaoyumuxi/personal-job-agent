...
import { api, label, date, Badge, type Start, type Send } from "./shared.js";
import { presentJob, runForJob, isActiveRun } from "./jobPresentation.js";
...
export function TaskWorkspace({
  ...
  const [history, setHistory] = useState<
    ...
    [acceptedRequest, setAcceptedRequest] = useState<string>();
  useEffect(() => {
    let ok = true;
    setHistory([]);
    ...
    if (row)
      void api
        .invoke({ method: "history", jobId: row.job.id })
        .then((v) => {
  ...
  const active = !!run && isActiveRun(snapshot, run);
  const q = active ? run.request : undefined;
  ...
  const presentation = row ? presentJob(row, snapshot) : undefined;
  const profile =
    run?.profile ??
    (row?.application?.profileId
      ? {
          id: row.application.profileId,
          name: row.application.profileName || row.application.profileId,
          revision: row.application.profileRevision,
        }
      : undefined);
  const phase =
    active && ["PAUSING", "PAUSED", "CANCELLING"].includes(run.state)
      ? label(run.state)
      : q?.kind === "confirm"
        ? "确认本次操作"
        : q?.kind === "login"
          ...
          : q?.kind === "review"
            ...
            : row?.application?.state === "UNKNOWN_RESULT"
              ...
              : run
                ? label(run.state)
                : presentation?.title || "查看任务记录";
  return (
    <article className="task-workspace" aria-label="任务工作区">
      <button className="back-link" onClick={close}>
        ← 返回工作台
      </button>
      <header className="page-head">
        <div>
          <div className="eyebrow">投递工作台 / 任务工作区</div>
          <h1 ref={heading} tabIndex={-1}>
            {row?.job.company || "工作空间任务"}
          </h1>
          <p>
            {row?.job.title || label(run?.operation)}
            {row?.job.url && <span className="task-url"> · {row.job.url}</span>}
          </p>
        </div>
        <span className="stage-label">{phase}</span>
      </header>
      <div className="workflow-guide" aria-label="申请流程">
        <span>准备资料</span>
      ...
      <div className="task-layout">
        <div className="task-primary">
          {q && (
            <section className="panel request" key={q.requestId}>
              ...
              {q.kind === "confirm" && (
                <label className="check">
                  <input
                    type="checkbox"
                    checked={acceptedRequest === q.requestId}
                    disabled={blocked}
                    onChange={(e) =>
                      setAcceptedRequest(
                        e.target.checked ? q.requestId : undefined,
                      )
                    }
                  />
                  我已核对上述目标与本次操作范围
              ...
              {q.kind === "confirm" && (
                <div className="actions">
                  <button
                    className="primary"
                    disabled={blocked || acceptedRequest !== q.requestId}
              ...
              {q.kind === "review" && (
                <>
                  ...
                  <div className="review-box">
                    ...
                    <p>
                      “前往官网”切换浏览器；“检查回执”只核查提交结果。没有可验证回执时，将记录“提交结果未知”，禁止重复填写。
                    </p>
                    <button
                      className="primary"
                      disabled={blocked}
            ...
            <>
              {row.application?.state === "UNKNOWN_RESULT" && (
                <div className="callout warning">
                  ...
                  <button
                    disabled={pending || snapshot.busy || snapshot.lock}
          ...
          {!q && (
            <section className="panel current-step">
              <div className="eyebrow">现在可以做什么</div>
              <h2>
                {active ? phase : presentation?.title || "查看本次任务记录"}
              </h2>
              <p>
                {active
                  ? run?.step
              ...
              {row &&
                !active &&
                row.job.url &&
                [undefined, "DRAFT", "FILLING", "REVIEW"].includes(
                  row.application?.state,
                ) && (
                  <button
                    className="primary"
                    disabled={
                    ...
                    onClick={() => start("apply", row.job.id)}
                  >
                    选择简历并准备填写
                  </button>
                )}
              {((snapshot.busy && !active) || (snapshot.lock && !active)) && (
                <p className="hint">
                  其他任务正在使用工作空间；结束后可执行新的操作。
                </p>
              )}
            </section>
          )}
          ...
          <p className="footnote">
            返回工作台或切换页面不会停止任务。最终提交始终由你在官网完成。
          </p>
        </div>
        <aside className="task-context" aria-label="资料与证据">
          <section className="panel">
            <h3>本次资料版本</h3>
            {profile ? (
              <>
                <strong>本次简历：{profile.name}</strong>
                <p className="hint">
                  修订 {profile.revision ?? "未记录"} · 任务绑定版本
                </p>
                <p className="hint">
                  资料页的默认版本切换不会改变本次申请已绑定的资料。
                </p>
              </>
            ) : (
              <p className="hint">本次任务尚未绑定简历版本。</p>
            )}
          </section>
          {row && (
            <section className="panel">
              ...
              <dl className="detail-list">
                <dt>执行状态</dt>
                <dd>
                  <Badge value={row.application?.state || "DRAFT"} />
                </dd>
                <dt>官网登录</dt>
                <dd>
                  <Badge value={row.application?.authStatus || "UNKNOWN"} />
                </dd>
                <dt>最近尝试</dt>
                <dd>{date(row.application?.lastAttempt)}</dd>
                <dt>最近成功查询</dt>
                <dd>{date(row.application?.lastSuccess)}</dd>
              </dl>
              <dl className="detail-list">
                <dt>招聘阶段</dt>
          ...
          {row && (
            <section className="panel">
              <h3>招聘信息</h3>
              {row.job.importWarning && !row.job.url && (
                <p className="callout warning">{row.job.importWarning}</p>
              )}
              <dl className="detail-list recruitment-details">
                <dt>招聘批次</dt>
                <dd>{row.job.batch || "未标注"}</dd>
                {row.job.referral && (
                  <>
                    <dt>内推码 / 内推信息</dt>
                    <dd>{row.job.referral}</dd>
                  </>
                )}
                {row.job.note && (
                  <>
                ...
                {Object.entries(row.job.sourceFields ?? {}).map(
                  ([name, value]) => (
                    <div className="source-field" key={name}>
                      <dt>{name}</dt>
                      <dd>{value}</dd>
                    </div>
                  ),
          ...
          <section className="panel">
            ...
            {run && !snapshot.events.some((e) => e.runId === run.runId) && (
              <p className="hint">本次任务尚无事件记录。</p>
            )}
            {!run &&
              history.map((e) => (
            ...
            {historyError && (
              <p className="error-text">任务历史读取失败，请返回后重试。</p>
            )}
            {!run && !history.length && !historyError && (
              <p className="hint">还没有任务事件。</p>
            )}
          </section>
        </aside>
      </div>
